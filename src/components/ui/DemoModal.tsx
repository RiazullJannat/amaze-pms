      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            onClick={closeModal}
            className="fixed inset-0 z-[60] bg-[#0a0f1d]/75 backdrop-blur-md"
          />

          {/*
           * RESPONSIVE PANEL:
           * – Mobile (< sm): bottom sheet — slides in from bottom,
           *   full width, rounded top corners only, max 90dvh, scrollable
           * – Desktop (sm+): centered dialog, max-w-lg, fully rounded
           */}
          ...
          >
            <div
              className="relative w-full rounded-t-3xl border border-slate-700/60 border-b-0 bg-[#0d1424]/97 backdrop-blur-2xl shadow-[0_-24px_80px_rgba(0,0,0,0.6)] flex flex-col max-h-[92dvh]"
            ...
            >
              ...
              {/* Drag handle + close */}
              <div className="relative z-10 flex items-center justify-between pt-4 pb-2 px-5 flex-shrink-0">
                <div className="w-10 h-1 rounded-full bg-slate-600 mx-auto absolute left-1/2 -translate-x-1/2 top-3" />
                <div className="ml-auto">
                  <button
                    onClick={closeModal}
                    className="p-2 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/10 transition-all"
              ...
              <div className="relative z-10 overflow-y-auto overscroll-contain px-5 pb-8 pt-2 flex-1">
                <ModalContent
                  success={success}
                  form={form}
                  errors={errors}
          ...
          <motion.div
            ...
            className="fixed inset-0 z-[61] items-center justify-center p-6 hidden sm:flex"
            aria-modal="true"
            role="dialog"
            aria-labelledby="demo-modal-title"
            onClick={closeModal}
...
/* ── Shared modal body (form + success state) ── */
...
function ModalContent({
  success, form, errors, touched, submitting,
  handleChange, handleBlur, handleSubmit, closeModal,
}: ModalContentProps) {
  return (
